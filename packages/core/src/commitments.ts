...
export function quoteCommitment(batch: BatchSnapshot, input: CommitmentInput): DomainResult<CommitmentQuote> {
  ...
  if (batch.status !== "OPEN" && batch.status !== "FUNDED") {
    return {
      ok: false,
      error: {
        code: "BATCH_NOT_ACCEPTING_COMMITMENTS",
        message: "This batch is not accepting buyer commitments.",
        detail: { status: batch.status },
      },
    };
  }

  if (input.now > batch.deadlineAt) {
    return {
      ok: false,
      error: {
        code: "BATCH_DEADLINE_PASSED",
        message: "This batch has passed its commitment deadline.",
      },
    };
  }
  ...
  if (!Number.isInteger(input.quantity) || input.quantity <= 0) {
    return {
      ok: false,
      error: {
        code: "INVALID_COMMITMENT_QUANTITY",
        message: "Commitment quantity must be a positive whole number.",
      },
    };
  }
  ...
  return {
    ...
    data: {
      ...
      totalAmount: roundMoney(input.quantity * input.unitPrice),
      currency: input.currency,
      lockState: batch.status === "FUNDED" ? "LOCKED_AFTER_CLEARING" : "CANCELLABLE_UNTIL_CLEARING",
    },
  };
}
