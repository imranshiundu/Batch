...
export function assertTransition(from: BatchStatus, to: BatchStatus): DomainResult<{ from: BatchStatus; to: BatchStatus }> {
  if (!canTransition(from, to)) {
    return { ok: false, error: { code: "INVALID_BATCH_TRANSITION", message: `Cannot transition batch from ${from} to ${to}.`, detail: { from, to, allowed: transitions[from] ?? [] } } };
  }
...
export function nextAutomaticStatus(batch: BatchSnapshot, now: Date): DomainResult<BatchStatus> {
  if (batch.status === "OPEN") {
    if (batch.committedUnits >= batch.minimumUnits && batch.committedAmount >= batch.minimumAmount) return { ok: true, data: "FUNDED" };
    if (now > batch.deadlineAt) return { ok: true, data: "FAILED" };
  }
  if (batch.status === "FUNDED") return { ok: true, data: "SUPPLIER_CONFIRMING" };
  if (batch.status === "SUPPLIER_CONFIRMING") {
    if (batch.supplierConfirmed) return { ok: true, data: "ACTIVE" };
    if (batch.supplierConfirmationDueAt && now > batch.supplierConfirmationDueAt) return { ok: true, data: "FAILED" };
  }
  return { ok: true, data: batch.status };
}

export function requireNonBlockedRisk(batch: BatchSnapshot): DomainResult<true> {
  if (batch.riskLevel === "BLOCKED") return { ok: false, error: { code: "BATCH_RISK_BLOCKED", message: "This batch is blocked by risk controls." } };
